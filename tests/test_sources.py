...
def test_no_queda_ninguna_fuente_apagada():
    """El registro solo lleva medios que se han visto publicar de verdad.

    Una fuente apagada es una promesa incumplida esperando a que alguien la
    encienda sin volver a medirla; las que no rendian se han retirado.
    """
    assert [f.clave for f in sources.FUENTES if not f.activa] == []
...
def test_fuente_de_resuelve_por_host():
    assert sources.fuente_de("https://www.marca.com/x").clave == "marca"
